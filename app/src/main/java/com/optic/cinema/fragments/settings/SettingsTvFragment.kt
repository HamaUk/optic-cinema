...
import com.optic.cinema.activities.main.MainTvActivity
import com.optic.cinema.backup.BackupRestoreManager
import com.optic.cinema.backup.ProviderBackupContext
import com.optic.cinema.database.AppDatabase
import com.optic.cinema.database.dao.EpisodeDao
import com.optic.cinema.database.dao.MovieDao
import com.optic.cinema.database.dao.TvShowDao
import com.optic.cinema.database.dao.SeasonDao
import com.optic.cinema.providers.FrenchStreamProvider
import com.optic.cinema.providers.Provider
import com.optic.cinema.providers.ProviderConfigUrl
import com.optic.cinema.providers.ProviderPortalUrl
import com.optic.cinema.providers.StreamingCommunityProvider
...
import com.optic.cinema.utils.WebSocketBypassTestHelper
    ...
    private fun displaySettings() {
        ...
        findPreference<Preference>("p_settings_help")?.setOnPreferenceClickListener {
            startActivity(
                Intent(
                    Intent.ACTION_VIEW,
                    Uri.parse("https://github.com/Optic Cinema-reborn/Optic Cinema")
                )
            )
            true
        }
        ...
        findPreference<Preference>("key_backup_export_tv")?.setOnPreferenceClickListener {
            ...
            val fileName = "Optic Cinema_tv_backup_$timestamp.json"
        ...
        findPreference<Preference>("key_backup_export_db_tv")?.setOnPreferenceClickListener {
            val timestamp = SimpleDateFormat("yyyyMMdd_HHmmss", Locale.getDefault()).format(Date())
            val fileName = "Optic Cinema_tv_db_backup_$timestamp.zip"
            ...
            withBackupLoading(R.string.backup_export_title) {
                ...
                runCatching {
                    val values = ContentValues().apply {
                        ...
                        put(MediaStore.Downloads.RELATIVE_PATH, Environment.DIRECTORY_DOWNLOADS + "/Optic Cinema")
                    }
                    ...
                    Toast.makeText(
                        requireContext(),
                        getString(R.string.backup_export_saved_to, "Downloads/Optic Cinema/$fileName"),
                        Toast.LENGTH_LONG
                    ).show()
                }.onFailure { error ->
                    Toast.makeText(requireContext(), getString(R.string.backup_export_error_write), Toast.LENGTH_LONG).show()
